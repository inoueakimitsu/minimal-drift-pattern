...
**Minimal Drift Pattern**（最小漂移模式）是一种通用方法，能够在保持源（S）与目标（D）之间对应关系的同时，以最小"差异"的方式反映新的变更。
它强调一种具有"惯性"的编辑方式，在更新过程中尽可能保持原有内容（翻译、注释、图像等）的意图和结构。
...
- **最小漂移模式**旨在让每次的变更都保持在尽可能小的范围，从而持续维持或快速修复 `Consistent` 状态。
...
1. 初始时 $s_0$ 和 $d_0$ 保持一致（`Consistent = 1`）
...
- **Pattern Name:** Minimal Drift Pattern (最小漂移模式)
